...
class PseudoConn
  ...
  DEFAULTS = { :transport => :tcp, :ack => false, :mtu => 1500,
               ...
               :timestamp => nil, :delta => nil,
               :src_ip => "10.0.0.1", :dst_ip => "42.13.37.80" }

  def PseudoConn.merge_opts(baseline, delta)
    res = baseline.dup
  ...
  def PseudoConn.connect(*opts)
    @@body ||= String.new
    ret = PseudoConn.new
    ret.opts = merge_opts(DEFAULTS, opts.first)
    ret.choose_sides

    # Three-way handshake if necessary
    if ret.opts[:transport] == :tcp
      ret.frame(:client, nil, :syn)
      ret.frame(:server, nil, :syn_ack)
      ret.frame(:client, nil, :ack)
    end
    return ret
  end

  def PseudoConn.write_pcap(filename)
  ...
  def frame(direction, data, *flags)
    ...
    if direction == :server || direction == :dst || !direction || direction == 0
      src, dst = 0, 1
    else
      src, dst = 1, 0
    end
    ...
    # Frame header
    if @opts[:delay]
      @timestamp += (delay.to_f / 1000)
    else
      @timestamp = Time.now
    end 
    hdr = itohl(@timestamp.to_i)
    hdr << itohl((@timestamp.to_f.remainder(1) * 1000000).to_i)
  ...
  attr_accessor :port, :mac, :ip, :seq, :body, :opts
...
# Some testing script
if __FILE__ == $0
  conn = PseudoConn.connect(:src_seq => 2**32 - 10, :dst_port => 80)
  conn.client("GET /index.html HTTP/1.0\r\nHost: fake.com\r\n\r\n")
  conn.server("HTTP/1.0 200 OK\r\nContent-Length: 5\r\n\r\nabcde")
  conn.close
  conn.write_pcap('/tmp/bob.pcap')
end
